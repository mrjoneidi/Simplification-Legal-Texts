...
class UnlimiformerT5(Unlimiformer[T5Model]):
    ...
    def activation_to_capture(self, layer_begin, layer_end):
        """
        Capture activations from the encoder and decoder layers
        (T5 has both encoder and decoder blocks).
        You can customize this to capture from specific layers.
        """
        encoder_layers = [
            self.model.encoder.block[i] for i in range(layer_begin, layer_end)
        ]
        decoder_layers = [
            self.model.decoder.block[i] for i in range(layer_begin, layer_end)
        ]
